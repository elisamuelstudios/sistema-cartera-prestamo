    ...
    <app-modal [open]="modal()" [wide]="true" [title]="editing()?'Editar cliente':'Registrar cliente'" (closed)="closeModal()">
      ...
      <form [formGroup]="form" (ngSubmit)="save()">
        ...
        @if(step()===3){<div class="form-grid">
          ...
          <div class="field"><label>Ruta asignada</label><select formControlName="routeId"><option value="">Sin asignar</option>@for(route of routes();track route.id){<option [value]="route.id">{{route.code}} · {{route.name}}</option>}</select></div>
