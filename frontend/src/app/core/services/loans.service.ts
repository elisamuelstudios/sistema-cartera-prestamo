...
@Injectable({ providedIn: 'root' }) export class LoansService {
  ...
  get(id:string){return this.api.get<Loan>(`loans/${id}`);} preview(body:unknown){return this.api.post<{interestRate:number;generatedInterest:number;total:number;dailyInstallment:number}>('loans/preview',body);}
