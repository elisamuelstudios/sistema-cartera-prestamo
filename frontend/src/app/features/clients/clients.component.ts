    ...
    <app-modal [open]="modal()" [wide]="true" [title]="editing()?'Editar cliente':'Registrar cliente'" (closed)="closeModal()">
      ...
      <form [formGroup]="form" (ngSubmit)="save()">
        ...
        @if(step()===3){<div class="form-grid">
          ...
          <div class="field span-2"><label>Referencias personales</label><textarea rows="2" formControlName="personalReferences"></textarea></div>
          <div class="field span-2"><label>Referencias familiares</label><textarea rows="2" formControlName="familyReferences"></textarea></div>
...
export class ClientsComponent implements OnInit{
  ...
  openNew(){this.editing.set(false);this.form.reset({firstNames:'',lastNames:'',identification:'',birthDate:'',primaryPhone:'',alternatePhone:'',address:'',neighborhood:'',city:'',email:'',occupation:'',workplace:'',monthlyIncome:0,status:'Activo',routeId:'',personalReferences:'',familyReferences:'',observations:''});this.step.set(1);this.modal.set(true)}
  edit(){const client=this.selected();if(!client)return;this.editing.set(true);this.service.get(client.id).subscribe(full=>{this.form.patchValue({firstNames:full.firstNames,lastNames:full.lastNames,identification:full.identification,birthDate:String(full['birthDate']??''),primaryPhone:full.primaryPhone??'',alternatePhone:String(full['alternatePhone']??''),address:String(full['address']??''),neighborhood:String(full['neighborhood']??''),city:String(full.city??''),email:String(full['email']??''),occupation:String(full['occupation']??''),workplace:String(full['workplace']??''),monthlyIncome:Number(full.monthlyIncome??0),status:full.status,routeId:String(full.routeId??''),personalReferences:String(full['personalReferences']??''),familyReferences:String(full['familyReferences']??''),observations:String(full['observations']??'')});this.step.set(1);this.modal.set(true)})}
