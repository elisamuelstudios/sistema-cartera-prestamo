...
import { Client, Loan, Payment } from '../../core/models/models';
...
@Component({
 ...
 template:`
  ...
  <section class="panel"><div class="toolbar"><div class="search"><input placeholder="Buscar recibo, préstamo o cliente…" (input)="changeSearch($event)"></div></div><div class="table-wrap"><table><thead><tr><th>Recibo</th><th>Fecha del pago</th><th>Registrado</th><th>Préstamo</th><th>Cliente</th><th>Valor pagado</th><th>Saldo pendiente</th><th>Método</th><th>Responsable</th></tr></thead><tbody>
   @for(payment of payments();track payment.id){<tr [class.selected]="selected()?.id===payment.id" (click)="selected.set(payment)" (dblclick)="openEdit()"><td><strong>{{payment.receipt}}</strong></td><td>{{payment.paymentDate|date:'dd/MM/yyyy'}}</td><td>{{payment.createdAt|appDateTime}}</td><td>{{payment.loanNumber}}</td><td>{{payment.clientName}}</td><td class="money"><strong>{{payment.amount|currency:'COP':'symbol-narrow':'1.0-0'}}</strong></td><td class="money balance">{{payment.pendingBalance|currency:'COP':'symbol-narrow':'1.0-0'}}</td><td>{{payment.method}}</td><td>{{payment.responsible||'—'}}</td></tr>}
...
export class PaymentsComponent implements OnInit{
 private service=inject(PaymentsService);private loanService=inject(LoansService);private clientService=inject(ClientsService);private toast=inject(ToastService);private fb=inject(FormBuilder);private timer?:number;
 readonly payments=signal<Payment[]>([]);readonly loans=signal<Loan[]>([]);readonly clients=signal<Client[]>([]);readonly selected=signal<Payment|null>(null);readonly selectedClient=signal<Client|null>(null);readonly selectedLoan=signal<Loan|null>(null);readonly page=signal(1);readonly total=signal(0);readonly search=signal('');readonly modal=signal(false);readonly editing=signal(false);readonly saving=signal(false);
 ...
 ngOnInit(){this.load()}load(){this.service.list(this.search(),this.page()).subscribe({next:r=>{this.payments.set(r.items);this.total.set(r.total)},error:e=>this.toast.error(e)})}go(p:number){this.page.set(p);this.load()}changeSearch(e:Event){this.search.set((e.target as HTMLInputElement).value);clearTimeout(this.timer);this.timer=window.setTimeout(()=>{this.page.set(1);this.load()},300)}
