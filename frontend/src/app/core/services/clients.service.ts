...
@Injectable({ providedIn: 'root' }) export class ClientsService {
  private api=inject(ApiService); list(search='',page=1,pageSize=25,status='',collectable=false,routeId=''){return this.api.get<Page<Client>>('clients',{search,page,pageSize,status,collectable:String(collectable),routeId});}
