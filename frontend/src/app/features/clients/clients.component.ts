...
@Component({
  ...
  template:`
    ...
    <section class="panel">
      <div class="toolbar"><div class="search"><input placeholder="Buscar por cualquier campo…" [value]="search()" (input)="changeSearch($event)"></div><select [value]="status()" (change)="changeStatus($event)"><option value="">Todos los estados</option><option>Activo</option><option>Lista Negra</option><option>Inactivo</option></select><select [value]="routeId()" (change)="changeRoute($event)"><option value="">Todas las rutas</option>@for(route of routes();track route.id){<option [value]="route.id">{{route.code}} · {{route.name}}</option>}</select></div>
...
export class ClientsComponent implements OnInit{
  ...
  readonly clients=signal<Client[]>([]);readonly routes=signal<Route[]>([]);readonly total=signal(0);readonly page=signal(1);readonly search=signal('');readonly status=signal('');readonly routeId=signal('');readonly loading=signal(false);readonly selected=signal<Client|null>(null);readonly modal=signal(false);readonly editing=signal(false);readonly step=signal(1);readonly saving=signal(false);
