...
export class ClientsController {
  ...
  @Get() findAll(@Query('search') search = '', @Query('status') status = '', @Query('page') page = '1', @Query('pageSize') pageSize = '25', @Query('collectable') collectable = 'false') {
    return this.service.findAll(search, status, Math.max(1, Number(page)), Math.min(100, Math.max(1, Number(pageSize))), collectable === 'true');
  }
